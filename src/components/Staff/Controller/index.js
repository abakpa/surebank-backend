...
const registerStaff = async (req, res) => {
  ...
  req.body.password = await bcrypt.hash(req.body.password,salt)
    const { name, phone, email, password, address,role,branch } = req.body;
    const existingStaff = await staffService.getStaffByEmail(email);
    if (existingStaff) {
      return res.status(400).json({ message: 'Staff already exists' });
    }
    const newStaff = await staffService.createStaff({ name, phone, email,password, address, password,role,branch });
